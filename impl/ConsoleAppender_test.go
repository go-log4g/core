...
func TestConsoleAppenderFilter(test *testing.T) {
	...
	appender := impl.NewConsoleAppender(buffer, layout, filter.NewThresholdFilter(slog.LevelError, filter.Neutral, filter.Deny), nil)

	info := slog.NewRecord(time.Now(), slog.LevelInfo, "Info", 0)
	errorRecord := slog.NewRecord(time.Now(), slog.LevelError, "Error", 0)

	appender.Append(&impl.LogEvent{Record: info})
	require.Empty(test, buffer.String())

	appender.Append(&impl.LogEvent{Record: errorRecord})
